...
export default function ResearchPage() {
  ...
  const cohortsNeedingBackfill = dashboard?.cohorts.filter((row) => row.coverage.backfill_required).length ?? 0;
  const outcomesAvailable = dashboard?.cohorts.reduce((total, row) => total + row.horizon_28d_available_count, 0) ?? 0;
  const incompletePaths = dashboard?.cohorts.filter((row) => !row.daily_path_review_complete).length ?? 0;
  const graphConnected = dashboard?.knowledge_graph.neo4j_connected === true;
  ...
  return (
    <main className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <StatCard label="Cohorts" value={String(dashboard?.cohorts.length ?? 0)} />
        <StatCard label="28D Outcomes" value={String(outcomesAvailable)} />
        <StatCard label="Backfill Needed" value={String(cohortsNeedingBackfill)} />
        <StatCard label="Neo4j" value={graphConnected ? "Connected" : "Check status"} />
      </div>
      ...
      {selected ? <>
        ...
        <Panel>
          <SectionTitle title="28D Outcomes" subtitle="Horizon availability is based on deterministic canonical price data, not on daily snapshot completeness." />
          <div className="overflow-auto">
            <table className="min-w-full text-left text-xs">
              <thead className="text-slate-400"><tr><th className="px-2 py-2">Symbol</th><th className="px-2 py-2">Outcome</th><th className="px-2 py-2">Return</th><th className="px-2 py-2">Daily Path</th><th className="px-2 py-2">Attribution / Regime</th><th className="px-2 py-2">DQ Flags</th></tr></thead>
              <tbody>{selected.outcomes.map((row) => <tr key={row.id} className="border-t border-stroke/60 align-top"><td className="px-2 py-2 font-medium">{row.symbol}<br /><span className="text-slate-500">{row.id.slice(0, 8)}</span></td><td className="px-2 py-2">{row.outcome_label}<br /><span className="text-slate-400">{row.outcome_date ?? "pending"}</span></td><td className="px-2 py-2">{pct(row.return_pct)}</td><td className="px-2 py-2">{row.daily_snapshot_path_complete ? "complete" : "incomplete"}<br /><span className="text-slate-400">{row.daily_snapshot_coverage_pct.toFixed(1)}%</span></td><td className="px-2 py-2">{row.attribution_label ?? "-"}<br /><span className="text-slate-400">{row.outcome_market_regime_id?.slice(0, 8) ?? "-"}</span></td><td className="px-2 py-2">{row.data_quality_flags.join(", ") || "none"}</td></tr>)}</tbody>
            </table>
          </div>
        </Panel>
